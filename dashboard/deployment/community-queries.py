...
def create_community_post(connection, author_id, author_username, title, content):
    ...
    cursor = connection.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (author_id, author_username, title, content, now_iso, now_iso),
    )
    connection.commit()
    return cursor.lastrowid


def list_community_posts(connection, limit=20, offset=0):
    rows = connection.execute(
        """
...
def count_community_posts(connection):
    return connection.execute("SELECT COUNT(*) FROM community_posts").fetchone()[0]


def get_community_post(connection, post_id):
    row = connection.execute(
