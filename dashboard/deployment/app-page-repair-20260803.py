...
def _site_map_links():
    ...
    links = [
        ...
        {
            "label": "게시판",
            "description": "버그 및 건의와 자유 게시판",
            "endpoint": "action_items_page",
            "children": [
...
def can_access_bug_report(item):
    return bool(
        item
        and item.get("source_type") == "bug_report"
        and (
    ...
    try:
        ...
        return render_template(
            ...
            checked_at=(
                ...
                if latest_law_sync and latest_law_sync.get("finished_at")
                else None
            ),
            changed_at=(
            ...
            check_status=(
                latest_law_sync.get("status") if latest_law_sync else None
            ),
        )
    finally:
        connection.close()
...
def company_news_page():
    ...
    try:
        ...
        for company in company_news:
            ...
            if selected_category != "all":
                articles = [
                    ...
                    if item["category_code"] == selected_category
                ]
        ...
        category_counts = {
            code: sum(
                len(company["articles"]) if code == "all"
                else company["counts"].get(code, 0)
                for company in company_news
...
def _community_board_context(connection, page=1, error=None, form_data=None):
    page_size = 20
    total = queries.count_community_posts(connection)
