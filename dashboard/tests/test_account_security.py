...
def test_webgl_bundle_is_limited_to_home(account_client):
    ...
    assert "profile-avatar-webgl.js" not in home
...
def test_account_does_not_load_blackhole_avatar_effect(account_client):
    ...
    assert 'id="blackhole-avatar-root"' not in html
    assert "/static/blackhole-hero/" not in html
    assert 'data-blackhole-avatar-fallback' not in html
    assert "profile-avatar-webgl.js" not in html
    assert "three.r149.min.js" not in html
