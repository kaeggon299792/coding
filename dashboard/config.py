...
RESEARCH_PDF_PROCESS_TIMEOUT_SECONDS = _get_int(
    "RESEARCH_PDF_PROCESS_TIMEOUT_SECONDS", 30
)
RESEARCH_PDF_PROCESS_MEMORY_MB = _get_int("RESEARCH_PDF_PROCESS_MEMORY_MB", 384)
RESEARCH_PDF_PROCESS_CPU_SECONDS = _get_int("RESEARCH_PDF_PROCESS_CPU_SECONDS", 20)
...
# Editor images can contain member- or admin-only material, so they must not
# live under PythonAnywhere's directly served /static mapping.  Keep the old
# setting as the migration source while the protected destination gets its own
# explicit setting.
EDITOR_IMAGE_LEGACY_DIR = _get_str(
    "EDITOR_IMAGE_DIR", str(BASE_DIR / "static" / "uploads" / "editor")
)
EDITOR_IMAGE_DIR = _get_str(
    "PROTECTED_EDITOR_IMAGE_DIR", str(BASE_DIR / "data" / "editor_images")
)
EDITOR_IMAGE_MAX_BYTES = _get_int("EDITOR_IMAGE_MAX_BYTES", 5 * 1024 * 1024)
DIARY_IMAGE_DIR = _get_str(
...
PORTFOLIO_SHAREPOINT_MAX_BYTES = _get_int(
    "PORTFOLIO_SHAREPOINT_MAX_BYTES", 20 * 1024 * 1024
)
PORTFOLIO_APP_DIR = _get_str("PORTFOLIO_APP_DIR", "/home/kaekun/portfolio")
PORTFOLIO_IMAGE_MAX_BYTES = _get_int(
    "PORTFOLIO_IMAGE_MAX_BYTES", 2 * 1024 * 1024
)
...
# PythonAnywhere account status (admin sidebar only).
PYTHONANYWHERE_USERNAME = _get_str("PYTHONANYWHERE_USERNAME")
PYTHONANYWHERE_API_TOKEN = _get_str("PYTHONANYWHERE_API_TOKEN")
PYTHONANYWHERE_DISK_QUOTA_GB = _get_optional_float("PYTHONANYWHERE_DISK_QUOTA_GB")
PYTHONANYWHERE_STATUS_CACHE_SECONDS = _get_int(
    "PYTHONANYWHERE_STATUS_CACHE_SECONDS", 300
)
PYTHONANYWHERE_API_TIMEOUT_SECONDS = _get_int(
    "PYTHONANYWHERE_API_TIMEOUT_SECONDS", 8
)
PYTHONANYWHERE_DISK_TIMEOUT_SECONDS = _get_int(
...
LOCALIZATION_TRANSLATION_LANGUAGES = _get_str(
    "LOCALIZATION_TRANSLATION_LANGUAGES", "en,ja,zh-CN,yue-HK"
)
...
ASSEMBLY_BILL_ARCHIVE_MAX_BYTES = _get_int(
    "ASSEMBLY_BILL_ARCHIVE_MAX_BYTES", 20 * 1024 * 1024
)
ASSEMBLY_BILL_PDF_MAX_BYTES = _get_int(
    "ASSEMBLY_BILL_PDF_MAX_BYTES", 40 * 1024 * 1024
)
ASSEMBLY_BILL_ZIP_MAX_ENTRIES = _get_int("ASSEMBLY_BILL_ZIP_MAX_ENTRIES", 40)
ASSEMBLY_BILL_ZIP_MAX_RATIO = _get_int("ASSEMBLY_BILL_ZIP_MAX_RATIO", 100)
ASSEMBLY_BILL_ZIP_MAX_UNCOMPRESSED_BYTES = _get_int(
    "ASSEMBLY_BILL_ZIP_MAX_UNCOMPRESSED_BYTES", 80 * 1024 * 1024
)
...
NPS_API_KEY = _get_str("NPS_API_KEY")
NPS_REQUEST_TIMEOUT_SECONDS = _get_int("NPS_REQUEST_TIMEOUT_SECONDS", 30)
