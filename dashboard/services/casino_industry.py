...
OPERATOR_CODES = {
    "㈜파라다이스": "paradise",
    "그랜드코리아레저㈜": "gkl",
    "㈜파라다이스세가사미": "paradise_segasammy",
    "㈜인스파이어 인티그레이티드 리조트": "inspire",
    "㈜지바스": "gvas",
    "㈜골든크라운": "golden_crown",
    "길상창휘(유)": "gilsang_changhwi",
    "㈜청해": "cheonghae",
    "㈜건하": "geonha",
    "㈜엘티엔터테인먼트": "lotte_tour",
    "헤븐㈜": "heaven",
    "람정엔터테인먼트코리아㈜": "landing_entertainment",
    "㈜금산": "geumsan",
}
...
def list_operator_companies() -> list[dict]:
    """카지노업 현황의 영업장을 운영 법인 단위로 묶어 반환한다."""
    operators = []
    by_code = {}
    for casino in CASINOS:
        company_name = casino["company_name"]
        entity_code = OPERATOR_CODES[company_name]
        operator = by_code.get(entity_code)
        if operator is None:
            operator = {
                "entity_code": entity_code,
                "entity_name": OPERATOR_DISPLAY_NAMES.get(entity_code, company_name),
                "operator_name": company_name,
                "venue_names": [],
                "regions": [],
            }
            by_code[entity_code] = operator
            operators.append(operator)
        operator["venue_names"].append(casino["venue_name"])
        if casino["region"] not in operator["regions"]:
            operator["regions"].append(casino["region"])
    for operator in operators:
        operator["venue_count"] = len(operator["venue_names"])
    return operators
